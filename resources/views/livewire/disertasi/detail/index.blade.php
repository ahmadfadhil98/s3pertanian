                            ...
                            <div x-show="opened_tab=={{$key}}">
                                <div class="bg-gray-50 rounded-md px-7 py-5 shadow-inner mb-5">
                                    <div class="w-full md:w-2/3 text-xl pb-1">
                                        ...
                                        @foreach ($c_academic as $c)
                                            @if($c->id==$proses_disertasi->id)
                                                @php
                                                     $no = 1;
                                                @endphp
                                            @endif
                                    ...
                                    @foreach ($c_academic as $count)
                                        @if ($count->id==$proses_disertasi->id)
                                            <div class="flex border-b-2 border-gray-200 py-2">
                                    ...
                                    @foreach ($c_academic as $count)
                                        @if ($count->id==$proses_disertasi->id)
                                            @foreach ($f_academics as $academic)
                                                @if($academic->proses_disertasi_id==$proses_disertasi->id)
                                                    <div class="flex mx-8 py-2 border-b-2 border-gray-200">
                                                        <div class="flex-grow">{{$academic->file_link}}</div>
                                    ...
                                    @foreach ($c_academic as $count)
                                        @if ($count->id==$proses_disertasi->id)
                                            @foreach ($l_academics as $academic)
                                                @if($academic->proses_disertasi_id==$proses_disertasi->id)
                                                    <div class="flex mx-8 py-2 border-b-2 border-gray-200">
                                                        <div class="flex-grow"><a href="{{$academic->file_link}}">{{$academic->file_link}}</a></div>
                                                        <div class="flex-none"></div>
                                                        <div class="flex-none">
                                                            <div class="flex">
                                                                <a href="{{$academic->file_link}}" class="transform hover:scale-95 duration-300 text-sm font-bold text-gray-500 focus:outline-none mr-3">
                                                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                ...
                                @can('admin_manage')
                                    <section class="flex justify-end">
                                        ...
                                        wire:click="academic({{$proses_disertasi->id}},'F')"
                                        ...
                                        <button style="background-color: #078CAA;" type="button"
                                        wire:click="academic({{$proses_disertasi->id}},'L')"
                                        class="transform hover:scale-95 duration-300 flex rounded-xl text-sm text-white py-3 px-6 focus:outline-none shadow-md mr-3">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" />
                                            </svg>
