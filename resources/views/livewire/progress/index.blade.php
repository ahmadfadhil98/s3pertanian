<div>
    <div class="max-w-screen-xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="flex mt-6">
                    ...
                    <button style="background-color: #078CAA;" wire:click="showModal()" class="transform hover:scale-95 duration-300 rounded-xl focus:outline-none py-2.5 px-7 text-base text-white shadow-md">
                        <div class="flex">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                              ...
                              <div class="ml-2.5">Tambah Progress Mahasiswa</div></div>
                            </button>
                        </div>
                ...
                @if($isOpen)
                    @include('livewire.progress.form')
                @endif

                @if($isDel)
                    @include('livewire.progress.delete')
                @endif
                ...
                <table class="table-fixed w-full mt-6">
                        ...
                        <tr>
                            ...
                            <th style="background-color: #057954;" class="font-normal text-base py-2.5 text-white rounded-tr-xl rounded-br-xl w-auto">
                                <div class="flex">
                                    ...
                                    <div class="ml-3">Nama Matkul</div>
                                </div>
                            </th>
                            <th class="w-48"></th>
                        </tr>
                    ...
                    <tbody>
                        @foreach($progress as  $key=>$progres)
                            <tr>
                                <td class="text-center text-base text-gray-600 py-4">({{ $progress->firstitem() + $key }})</td>
                                <td class="text-left text-base text-gray-600">
                                        {{ $students[$progres->student_id] }}
                                </td>
                                <td class="text-left text-base text-gray-600">
                                    @foreach ($progMat as $item)
                                        @if ($item->student_id == $progres->student_id)
                                            {{ $matkuls[$item->proses_disertasi_id] }}
                                            <br>
                                        @endif
                                    @endforeach
                                </td>
                                ...
                                <td class="text-right">
                                    <button style="background-color: #078CAA;" wire:click="edit({{ $progres->id }})"class="transform hover:scale-95 duration-300 rounded-full text-sm font-bold text-white py-3 px-3 focus:outline-none shadow-md mr-1.5">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                        </svg>
                                    </button>
                                    <button style="background-color: #E42025;" wire:click="showDel({{ $progres->id }})"class="transform hover:scale-95 duration-300 rounded-full text-sm font-bold text-white py-3 px-3 focus:outline-none shadow-md">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div class="mt-4">
                    {{$progress->links('pagination_section')}}
                </div>
    </div>
</div>
