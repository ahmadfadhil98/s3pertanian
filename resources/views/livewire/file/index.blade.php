    ...
    <div class="max-w-screen-xl mx-auto sm:px-6 lg:px-8">
        <div class="flex mt-7">
            ...
            <div class="flex mt-6">
                ...
                <div class="w-full md:w-1/2">
                    <input wire:model="search" type="text" class="w-full focus:outline-none py-3 px-6 rounded-full text-gray-600" placeholder="Ketik nama file...">
                </div>
            </div>
                @if($isOpen)
                    ...
                    <tbody>
                        @foreach($files as  $key=>$file)
                            <tr>
                                ...
                                <td class="text-right">
                                    <button wire:click="download({{ $file->id }})" class="rounded-full text-sm font-bold bg-yellow-300 hover:bg-yellow-400 text-gray-600 py-2.5 px-7">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                                        </svg>
                                    </button>
                                    <button wire:click="edit({{ $file->id }})" class="rounded-full text-sm font-bold bg-green-500 hover:bg-green-700 text-white py-2.5 px-7 ml-1.5">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                        </svg>
                                    </button>
                                    <button wire:click="showDel({{ $file->id }})" class="rounded-full text-sm font-bold bg-red-500 hover:bg-red-700 text-white py-2.5 px-7 ml-1.5">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
