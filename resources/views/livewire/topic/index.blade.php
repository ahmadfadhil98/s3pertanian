        ...
        <div class="flex mt-7">
            ...
            <div class="flex mt-6">
                ...
                <div class="w-full md:w-1/2 flex text-gray-300 bg-white pl-5 rounded-xl shadow-md">
                    ...
                    <input wire:model="search" type="text" class="w-full focus:outline-none py-3 pl-3 text-gray-600 placeholder-gray-300 rounded-xl" placeholder="Ketik topik disertasi...">
                </div>
            </div>
                @if($isOpen)
                    ...
                    <tbody>
                        @foreach($topics as  $key=>$topic)
                            <tr>
                                ...
                                <td class="text-left text-base text-gray-600 font-bold px-6">{{ $topic->name }}</td>
                                <td class="text-right">
                                    <button wire:click="edit({{ $topic->id }})"class="rounded-xl text-sm font-bold bg-green-500 hover:bg-green-700 text-white py-2.5 px-7 focus:outline-none shadow-md">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                        </svg>
                                    </button>
                                    <button wire:click="showDel({{ $topic->id }})"class="ml-1.5 rounded-xl text-sm font-bold bg-red-500 hover:bg-red-700 text-white py-2.5 px-7 focus:outline-none shadow-md">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
