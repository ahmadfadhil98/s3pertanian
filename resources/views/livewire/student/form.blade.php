        ...
        <div class="inline-block align-bottom bg-white text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full" role="dialog" aria-modal="true" aria-labelledby="modal-headline">
            <form>
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <div>
                        <h1 class="font-bold text-center mb-4">INPUT MAHASISWA</h1>
                    </div>
                <div>
                <div class="mb-2">
                    <input wire:model="studentId" type="hidden" class="shadow appearance-none border w-full py-2 px-3 text-blue-900">
                </div>
                <div class="mb-2">
                    <label for="nim" class="block py-1">NIM</label>
                    <input wire:model="nim" type="number" name="nim" class="shadow appearance-none border w-full py-2 px-3 text-blue-900 text-sm" placeholder="Input NIM mahasiswa">
                    @error('nim') <h1 class="text-red-500">{{$message}}</h1>@enderror
                </div>
                <div class="mb-2">
                    <label for="name" class="block py-1">Nama</label>
                    <input wire:model="name" name="name" class="shadow appearance-none border w-full py-2 px-3 text-blue-900 text-sm" placeholder="Input nama mahasiswa">
                    @error('name') <h1 class="text-red-500">{{$message}}</h1>@enderror
                </div>
                <div class="mb-2">
                    <label for="email" class="block py-1">Email</label>
                    <input wire:model="email" type="email" name="email" class="shadow appearance-none border w-full py-2 px-3 text-blue-900 text-sm" placeholder="Input email mahasiswa">
