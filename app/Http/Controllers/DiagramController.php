...
class DiagramController extends Controller
{
    ...
    public function index(){
    	...
    	$subitem = new stdClass();
        $dasubitem = new stdClass();
        $subitem->jumlah = Student::where('status','A')->count();
        $students = Student::where('status','A')->select(DB::raw('SUBSTRING(nim,1,2) as tahun'),DB::raw('count(SUBSTRING(nim,1,2)) as jumlah'))->groupBy('tahun')->get();
        $jmlh_mhs = [];
        foreach($tahun as $key=>$tem){
            foreach($students as $student){
                if(substr($tem,2,4) == $student->tahun){
