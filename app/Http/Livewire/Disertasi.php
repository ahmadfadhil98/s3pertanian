...
class Disertasi extends Component
{
    ...
    public function render()
    {
        ...
        if($user->type==1){
            $disertasis = DB::table('disertasis')->join('students','students.id','=','disertasis.student_id')->select('disertasis.id','student_id','name','nim','topic_id','title','disertasis.status')->where('name','like',$searchParam)->orWhere('nim','like',$searchParam)->orWhere('title','like',$searchParam)->paginate(2);
        }else{
            $disertasis = DB::table('disertasis')->join('students','students.id','=','disertasis.student_id')->select('disertasis.id','student_id','name','nim','topic_id','title','disertasis.status')->where('student_id',$user->id)->paginate(2);
        }
